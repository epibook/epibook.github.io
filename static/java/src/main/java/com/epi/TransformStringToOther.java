...
For example, if the dictionary is $\{\mbox{bat}, \mbox{cot}, \mbox{dog}, \mbox{dag}, \mbox{dot}, \mbox{cat}\}$,
then $\langle \mbox{cat}, \mbox{cot}, \mbox{dot}, \mbox{dog}\rangle$ is production sequence.

@problem
Given a dictionary $D$ and two strings $s$ and $t$, write a program to determine if
$s$ produces $t$. Assume that all characters are lowercase alphabets.
If $s$ does produce $t$, output the length of a shortest production sequence; otherwise, output $-1$.
