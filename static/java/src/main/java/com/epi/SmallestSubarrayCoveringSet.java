...
public class SmallestSubarrayCoveringSet {
  ...
  public static Subarray findSmallestSubarrayCoveringSet(List<String> paragraph,
    ...
    for (int left = 0, right = 0; right < paragraph.size();) {
      ...
      // all keywords.
      for (;
           right < paragraph.size() && keywordsToCount.size() < keywords.size();
           ++right) {
        if (keywords.contains(paragraph.get(right))) {
          keywordsToCount.put(
              paragraph.get(right),
              keywordsToCount.containsKey(paragraph.get(right))
                  ? keywordsToCount.get(paragraph.get(right)) + 1
                  : 1);
        }
      }
      ...
      // have all keywords.
      for (; left < right && keywordsToCount.size() == keywords.size();
           ++left) {
        if (keywords.contains(paragraph.get(left))) {
          int keywordCount = keywordsToCount.get(paragraph.get(left));
          keywordsToCount.put(paragraph.get(left), --keywordCount);
          if (keywordCount == 0) {
            keywordsToCount.remove(paragraph.get(left));
