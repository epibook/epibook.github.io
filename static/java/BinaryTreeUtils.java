...
public class BinaryTreeUtils {
  public static BinaryTreeNode<Integer> generateRandBinaryTree(
    ...
    BinaryTreeNode<Integer> root
        = new BinaryTreeNode<>(isUnique ? n-- : r.nextInt(temp_n));
    l.add(root);
    while (n-- > 0) {
      ...
      if (addLeft && it.left == null || !addLeft && it.right == null) {
        it.left = new BinaryTreeNode<>(isUnique ? n : r.nextInt(temp_n));
        l.add(it.left);
      } else {
        it.right = new BinaryTreeNode<>(isUnique ? n : r.nextInt(temp_n));
        l.add(it.right);
      }
