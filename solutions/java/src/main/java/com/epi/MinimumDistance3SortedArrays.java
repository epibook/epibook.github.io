...
public class MinimumDistance3SortedArrays {
  ...
  public static int findMinDistanceSortedArrays(
    ...
    for (int i = 0; i < sortedArrays.size(); ++i) {
      if (idx.get(i) >= sortedArrays.get(i).size()) {
        return minDis;
      }
      currentHeads.add(new ArrayData(i, sortedArrays.get(i).get(idx.get(i))));
    }
    ...
    while (true) {
      ...
      if (idx.get(tar) >= sortedArrays.get(tar).size()) {
        return minDis;
      }
      currentHeads.pollFirst();
      currentHeads.add(new ArrayData(tar, sortedArrays.get(tar).get(idx.get(tar))));
    }
  }
  // @exclude
