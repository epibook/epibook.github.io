...
// @include
public class QueueWithMaxAlternative<T extends Comparable<T>> {
  ...
  private static <T extends Comparable<T>> void assertDequeue(
      QueueWithMaxAlternative<T> q, T t) {
    T dequeue = q.dequeue();
    assert(t.equals(dequeue));
  }

  public static void simpleTest() {
    QueueWithMaxAlternative<Integer> Q = new QueueWithMaxAlternative<>();
    Q.enqueue(11);
    Q.enqueue(2);
    assert(11 == Q.max());
    assertDequeue(Q, 11);
