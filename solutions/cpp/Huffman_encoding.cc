...
void AssignHuffmanCode(const shared_ptr<BinaryTreeNode>&, string*,
...
unordered_map<char, string> HuffmanEncoding(
  ...
  AssignHuffmanCode(candidates.top(), new string, &huffman_encoding);
  return huffman_encoding;
}

void AssignHuffmanCode(const shared_ptr<BinaryTreeNode>& tree, string* code,
                       unordered_map<char, string>* huffman_encoding) {
  if (tree) {
    if (tree->s) {
      // This node is a leaf.
