...
class BinarySearchTree {
 public:
  ...
  bool Insert(int key) {
    if (root_ == nullptr) {
      root_ = make_unique<BinaryTreeNode<int>>(
          BinaryTreeNode<int>{key, nullptr, nullptr});
    } else {
      ...
      if (key < parent->data) {
        ...
        parent->right.reset(new BinaryTreeNode<int>{key});
      }
    }
    return true;
  }
  ...
  bool Delete(int key) {
    ...
    BinaryTreeNode<int> *key_node = curr;
    if (key_node->right) {
      // Finds the minimum of the right subtree.
      BinaryTreeNode<int> *r_key_node = key_node->right.get(),
                          *r_parent = key_node;
