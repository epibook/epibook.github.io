...
int LongestContainedRange(const vector<int>& A) {
  ...
  for (int a : A) {
    if (unprocessed_entries.find(a) != unprocessed_entries.end()) {
      unprocessed_entries.erase(a);

      // Finds the lower bound of the largest range containing a.
      int lower_bound = a - 1;
      while (unprocessed_entries.find(lower_bound) !=
             unprocessed_entries.end()) {
      ...
      int upper_bound = a + 1;
      while (unprocessed_entries.find(upper_bound) !=
             unprocessed_entries.end()) {
