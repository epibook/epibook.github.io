...
string JoinALineWithBlank(const vector<string>&, size_t, size_t, size_t);
...
vector<string> JustifyText(const vector<string>& words, size_t L) {
  ...
  for (size_t i = 0; i < words.size(); ++i) {
    ...
    if (lookahead_line_length == L) {
      result.emplace_back(
          JoinALineWithBlank(words, curr_line_start, i, i - curr_line_start));
      curr_line_start = i + 1, num_words_curr_line = 0, curr_line_length = 0;
    } else if (lookahead_line_length > L) {
      result.emplace_back(JoinALineWithBlank(words, curr_line_start, i - 1,
                                             L - curr_line_length));
  ...
  if (num_words_curr_line > 0) {
    string line = JoinALineWithBlank(words, curr_line_start, words.size() - 1,
...
// Joins strings in words[start, end] with num_blanks blanks spread evenly.
string JoinALineWithBlank(const vector<string>& words, size_t start,
                          size_t end, size_t num_blanks) {
  size_t num_words_curr_line = end - start + 1;
  string line;
  for (size_t i = start; i < end; ++i) {
    line += words[i];
    --num_words_curr_line;
    size_t num_curr_blank =
        ceil(static_cast<double>(num_blanks) / num_words_curr_line);
    line.append(num_curr_blank, ' ');
    num_blanks -= num_curr_blank;
  }
  line += words[end];
